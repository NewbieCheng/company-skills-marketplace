...
把 `HGD1-` 设备请求码发给公司，收到本机专属 `HGL1-` 激活码后粘贴一次即可。完整流程见[授权版 Skill 小白指南](docs/LICENSED_SKILL_GUIDE.md)。
...
| `social-media-hangge-moments` | 社媒 | 设备授权后在 Codex/Cursor 中使用加密的航哥朋友圈工作流 | 无系统级依赖；需公司签发激活码 |
...
授权版包会由 Codex 根据 `catalog.json.delivery` 选择 Windows 或 macOS 安装器，再从 GitHub Release 下载经过 SHA-256 固定的加密交付包。用户仍然不需要手工下载或解压。
